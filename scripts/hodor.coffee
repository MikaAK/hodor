...
MMPhraseGenerator =
  phrase: ->
    "#{@adjective()} #{@name()} #{@verb()} #{@adverb()} #{@noun()}"
  name: ->
    @randomItemFrom @names
  noun: ->
    @randomItemFrom @nouns
  verb: ->
    @randomItemFrom @verbs
  adjective: ->
    @randomItemFrom @adjectives
  adverb: ->
    @randomItemFrom @adverbs
  company: ->
    @randomItemFrom @companies
  city: ->
    @randomItemFrom @cities
  usStateCode: ->
    @randomItemFrom @usStateCodes
...
hodorPossibilites = null
selectWords       = null

getRandom       = () -> Math.floor((Math.random() * 10) + 1)
getExclimations = () -> Array(getRandom()).join('!')
getPosibilites  = () ->
  hodorPossibilites = []
  hodorPossibilites.push('HODOR' + getExclimations()) for [0..10]
getPhrases      = () ->
  allPhrases = ''
  selectWords = []
...
setInterval getPosibilites, 3600000
setInterval getPhrases, 3600000
